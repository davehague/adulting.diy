...
export default defineProtectedEventHandler(async (event, authUser) => {
  try {
    ...
    const isAdmin = await householdService.isUserAdmin(authUser.userId, user.household_id);
    ...
    const category = await categoryService.create(body.name.trim(), user.household_id);
